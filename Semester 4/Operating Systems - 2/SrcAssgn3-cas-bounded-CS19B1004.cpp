...
//Entry section
	auto reqEnterTime = chrono::system_clock::now();
   	...
   	struct tm* time = localtime(&t_start);
        ...
        waiting_queue[tid-1] = true;
		int expected_lock_value = 0;
        key = 1;
		while(waiting_queue[tid-1] && key){
			key = !(lock.compare_exchange_weak(expected_lock_value, 1));
			expected_lock_value = 0;
		}
		waiting_queue[tid-1] = false;
...
//Critical section
	...
	usleep(ex_di_1(gen)*1000);
        ...
        int j = (tid)%n;
 		while(j!=tid-1 && !waiting_queue[j])
 			j = (j+1)%n;
 		if(j==tid-1)
 			lock = 0;
 		else
 			waiting_queue[j] = false;
...
int i;
ifstream input;
waiting_queue = (bool*)malloc(n*sizeof(bool));

input.open("inp-params.txt");
input >> n >> k >> l1 >> l2;
input.close();
output.open("Bounded_CAS.txt");
output << "Bounded CAS ME Output:"<<endl;
...
output1.open("Bounded-CAS-Stats.txt");
