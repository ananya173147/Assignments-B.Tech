      ...
      //if vertex belongs to current partition and is a boundary vertex
        if(partition[j]==id)          
            ...
            //apply greedy colouring
        		...
        		for (c=0; c<v; c++)
            		if (available[c] == false) //find lowest numbered coloured not used                             on prev coloured adj vertices
                		break;
...
// End keeping track of time taken
    ...
    output.open("output-coarse.txt");

  	output << "No. of colours used: " << no_colours+1 << endl;
  	output << "Time taken by coarse-grained lock algorithm: " << (double)time_spent/(double)1000 << " milliseconds"<< endl;
	  print();
	  output.close();
